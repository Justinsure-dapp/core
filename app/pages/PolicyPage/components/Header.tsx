...
export default function Header({ policy }: { policy: Policy }) {
  ...
  return (
    <div className="w-full flex flex-col items-end gap-2">
      <div className="relative w-full flex flex-col justify-between gap-1 bg-foreground/50 px-6 py-4">
        <div className="flex w-full gap-4 rounded-md">
          <div className="w-full max-w-[10rem] flex flex-col items-center gap-4">
            <img
              ...
              alt="Policy"
            />
            <p className="h-max md:hidden w-max rounded-xl border border-mute px-3 text-sm">
            ...
            <div className="flex flex-col gap-1">
              <div className="w-full flex flex-col gap-2 widescreen:flex-row justify-between">
                <div>
                  ...
                  <div className="flex mt-1 items-center rounded-xl">
                    <p className="mr-1">{calculateAverageRating({ ratings: policy?.ratings })}</p>
                ...
                <p className="h-max hidden md:flex w-max rounded-xl border border-mute px-3 mt-2 text-sm self-start">
                  Category: {policy.category}
                </p>
              </div>
...
function RatePolicy({ policy }: { policy: Policy }) {
  ...
  async function handleRatePolicy() {
    ...
    setLoading(true);
    try {
      const result = await api.policy.ratePolicy(policy.address, address, rating);
      ...
      console.error(error);
      error.response.data.message ? toast.error(error.response.data.message) : toast.error("Error occurred. Please try again.");
    }
  }
