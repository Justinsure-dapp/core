...
export default function WithdrawSurecoinModal() {
    ...
    const handleClaim = async () => {
        ...
        setLoading(true);
        const claimToast = toast("Processing claim...", { type: "info", isLoading: true });
        ...
        try {
            ...
            toast.update(claimToast, { render: "Transaction submitted!", type: "info", isLoading: false, autoClose: 2000 });
        } catch (error) {
            console.error(error);
            toast.update(claimToast, { render: "Transaction failed.", type: "error", isLoading: false, autoClose: 2000 });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="relative flex flex-col gap-y-1 bg-background w-[40vw] mobile:w-[80vw] px-8 py-8 rounded-lg border border-primary/60 mobile:px-8">
            ...
            <h1 className='text-lg'>Do you want to claim your Surecoins to your wallet?</h1>
            <p className='text-front/60 text-sm'>By following this process, you will be able to securely store Surecoins in your wallet, where they will remain stable. These Surecoins will be available for trade in exchange for USDJ at any time. </p>
            <div className='text-md justify-end gap-x-4 flex mt-4'>
                <button onClick={() => modal.hide()} className='bg-front/80 text-back py-1 px-4 rounded-sm'>Cancel</button>
                <button className={twMerge(
                    ...
                    onClick={handleClaim}
                    disabled={loading}>{loading ? "Claiming..." : "Claim"}</button>
            </div>
        </div>
    )
}
